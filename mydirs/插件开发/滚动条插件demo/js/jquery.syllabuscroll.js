	...
	$.fn.Mscrool = function(options) {
		...
		function mousefn() {
			BSidebar.mousedown(function() {
				...
				$(document).mousemove(function(event) {
					...
					bar.css('top', y);
					bil = y / tp
					s = bil * (Bcon.height() + Number(dft.Conwhite) - _this.height());
					Bcon.css('top', -s);
					return false;
				})
		...
		function mousewhfn() {
			_this.mousewheel(function(event, delta) {
				...
				function wheel() {
					v = delta * Number(dft.scrlspeed);
					lastT = conT + v;
					conh = Bcon.height() + Number(dft.Conwhite) - _this.height();
					if(lastT < -conh) {
						lastT = -conh;
					}
					if(lastT > 0) {
